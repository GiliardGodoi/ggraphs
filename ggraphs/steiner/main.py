...
from ..distance import shortest_path_one_to_all as dijkstra
from ..mst import prim, kruskal


def heuristic_shortest_path(graph, start, terminals):
    ...
    for t in terminals:
        pq.push(distancias[start][t], (start,t))
    ...
    while pq:
        ...
        if target not in distancias:
                ...
                for tmp in terminals:
                    pq.push(distancias[target][tmp], (target,tmp))
...
def heuristic_shortest_path_origin(graph, start, terminals):
    '''
...
def heuristic_shortest_path_origin_prim(graph, start, terminals):
    '''
...
def heuristic_prunning_mst(graph, start, terminals):
    '''
    Parameters:
        graph : Graph
...
def heuristic_prunning_kruskal(graph : Graph, terminals):
    """
    Parameters:
        graph : Graph
